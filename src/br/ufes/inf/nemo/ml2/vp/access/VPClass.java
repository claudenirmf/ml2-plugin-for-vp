...
//		};
	...
	public void addSuper(ML2Class ontoLClass) {
		VPClass superVPC = (VPClass) VPModelAccess.getModelElement(ML2ModelLoader.getFullyQualifiedName(ontoLClass));
	...
	public void setOrder(HOClass ho) {
		...
		for (int i=0; i < tvcount; i++) {
			if(tvalues[i].getName().equals(ML2ModelLoader.TG_VALLUE_ORDER)){
	...
	public void addDependencyTo(ML2Class c, String dependencyStereotype) {
		IDependency d = IModelElementFactory.instance().createDependency();
		VPDependency vpd = (VPDependency) VPModelElement.wrap(d);
		VPClass target = (VPClass) VPModelAccess.getModelElement(ML2ModelLoader.getFullyQualifiedName(c));
		vpd.setTarget(target);
		vpd.setSource(this);
		vpd.addStereotype(dependencyStereotype);
	}
	
	public void addInstantiationTo(ML2Class c) {
		addDependencyTo(c, ML2ModelLoader.STR_INSTANTIATION);
	}

	public void addSubordinatorClass(ML2Class c) {
		addDependencyTo(c, ML2ModelLoader.STR_SUBORDINATION);
	}
...
//	}
	...
	public void addBasetype(ML2Class c, CategorizationType catType) {
		switch (catType) {
		case CATEGORIZER:
			addDependencyTo(c,ML2ModelLoader.STR_CATEGORIZATION);
			break;
		case COMPLETE_CATEGORIZER:
			addDependencyTo(c,ML2ModelLoader.STR_COMPL_CATEGORZATION);
			break;
		case DISJOINT_CATEGORIZER:
			addDependencyTo(c,ML2ModelLoader.STR_DISJ_CATEGORIZATION);
			break;
		case PARTITIONER:
			addDependencyTo(c,ML2ModelLoader.STR_PARTITIONS);
			break;
		default:
			break;
		}
	...
	public void addPowertypeOf(ML2Class c) {
		addDependencyTo(c,ML2ModelLoader.STR_POWER_TYPE);
	}
	...
	public void addToAssociation(Reference reference) {
		...
		VPClass to = (VPClass) VPModelAccess.getModelElement(ML2ModelLoader.getFullyQualifiedName(reference.get_type()));
	...
	public boolean equals(Object obj) {
		if(obj instanceof EntityDeclaration){
			...
			String str2 = ML2ModelLoader.getFullyQualifiedName(entity);
	...
	public void addAttribute(Attribute att) {
		...
		String upperBound = att.getUpperBound()==-1 ? "*" : att.getUpperBound()+"" ;
		vp_att.getVPSource().setMultiplicity(att.getLowerBound()+".."+upperBound);
		
//		String fqn = ML2ModelLoader.getFullyQualifiedName(att.get_type());
